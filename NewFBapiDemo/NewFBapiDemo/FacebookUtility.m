...
-(NSArray *)getPermissionsArray
{
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        permissions = [[NSArray alloc] initWithObjects:@"user_birthday",@"email", @"publish_stream", @"offline_access",@"publish_actions",  @"user_location",  @"user_hometown",  @"user_about_me", @"user_relationships",@"user_address",@"user_relationships", nil];
    });
    return permissions;
}
...
{
    //[facebook requestWithGraphPath:@"me/friends?fields=username,name,picture" andDelegate:self];
    FBRequest* friendsRequest = [[FBRequest alloc]initWithSession:self.session graphPath:[NSString stringWithFormat:@"me/friends?fields=username,name,picture"]];
...
-(void)dealloc
{
    [_arrFBFriendList release];
    [_dictUserInfo release];
    [_session release];
    [super dealloc];
}
